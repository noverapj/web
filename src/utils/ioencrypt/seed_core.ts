...
// G function (inner part of the round function)
function G(X: number): number {
  let h: number;
  h = SEED_SL[0][X & 0xff];
  h = (h ^ SEED_SL[1][(X >> 8) & 0xff]) >>> 0;
  h = (h ^ SEED_SL[2][(X >> 16) & 0xff]) >>> 0;
  h = (h ^ SEED_SL[3][(X >> 24) & 0xff]) >>> 0;
  return h;
}
...
/**
 * Generates the round keys for SEED encryption/decryption.
 * @param UserKey - The 16-byte user key.
 * @param AlgInfo - The algorithm info object where RoundKey will be stored.
 * @returns Status code (CTR_SUCCESS or error code).
 */
export function SEED_KeySchedule(
  UserKey: Uint8Array,
  AlgInfo: SEED_ALG_INFO,
): number {
  if (UserKey.length !== SEED_USER_KEY_LEN) {
    return CTR_INVALID_USERKEYLEN;
  }

  let A = BIG_B2D(UserKey, 0);
  let B = BIG_B2D(UserKey, 4);
  let C = BIG_B2D(UserKey, 8);
  let D = BIG_B2D(UserKey, 12);
  const K = AlgInfo.RoundKey; // Reference to the array in AlgInfo
  ...
  let res: { A?: number; B?: number; C?: number; D?: number };

  res = EncRoundKeyUpdate0(K, 2, A, B, C, D, KC1);
  A = res.A!;
  B = res.B!;
  res = EncRoundKeyUpdate1(K, 4, A, B, C, D, KC2);
  C = res.C!;
  D = res.D!;
  res = EncRoundKeyUpdate0(K, 6, A, B, C, D, KC3);
  A = res.A!;
  B = res.B!;
  res = EncRoundKeyUpdate1(K, 8, A, B, C, D, KC4);
  C = res.C!;
  D = res.D!;
  res = EncRoundKeyUpdate0(K, 10, A, B, C, D, KC5);
  A = res.A!;
  B = res.B!;
  res = EncRoundKeyUpdate1(K, 12, A, B, C, D, KC6);
  C = res.C!;
  D = res.D!;
  res = EncRoundKeyUpdate0(K, 14, A, B, C, D, KC7);
  A = res.A!;
  B = res.B!;
  res = EncRoundKeyUpdate1(K, 16, A, B, C, D, KC8);
  C = res.C!;
  D = res.D!;
  res = EncRoundKeyUpdate0(K, 18, A, B, C, D, KC9);
  A = res.A!;
  B = res.B!;
  res = EncRoundKeyUpdate1(K, 20, A, B, C, D, KC10);
  C = res.C!;
  D = res.D!;
  res = EncRoundKeyUpdate0(K, 22, A, B, C, D, KC11);
  A = res.A!;
  B = res.B!;
  res = EncRoundKeyUpdate1(K, 24, A, B, C, D, KC12);
  C = res.C!;
  D = res.D!;
  res = EncRoundKeyUpdate0(K, 26, A, B, C, D, KC13);
  A = res.A!;
  B = res.B!;
  res = EncRoundKeyUpdate1(K, 28, A, B, C, D, KC14);
  C = res.C!;
  D = res.D!;
  res = EncRoundKeyUpdate0(K, 30, A, B, C, D, KC15);
  // Final A, B, C, D values are not used after this

  return CTR_SUCCESS;
}

// Helper for SeedRound logic
function SeedRound(
  A: number,
  B: number,
  C: number,
...
/**
 * Encrypts a single 16-byte block of data in place using SEED.
 * @param AlgInfo - Algorithm info containing the round keys.
 * @param Data - The 16-byte data block (Uint8Array) to encrypt.
 */
export function SEED_Encrypt(AlgInfo: SEED_ALG_INFO, Data: Uint8Array): void {
  const K = AlgInfo.RoundKey;
  let C = BIG_B2D(Data, 8);
  let D = BIG_B2D(Data, 12);
  let A = BIG_B2D(Data, 0);
  let B = BIG_B2D(Data, 4);
  let res: { A: number; B: number };

  res = SeedRound(A, B, C, D, K, 0);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 2);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 4);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 6);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 8);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 10);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 12);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 14);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 16);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 18);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 20);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 22);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 24);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 26);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 28);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 30);
  C = res.A;
  D = res.B;
...
/**
 * Decrypts a single 16-byte block of data in place using SEED.
 * @param AlgInfo - Algorithm info containing the round keys.
 * @param Data - The 16-byte data block (Uint8Array) to decrypt.
 */
export function SEED_Decrypt(AlgInfo: SEED_ALG_INFO, Data: Uint8Array): void {
  const K = AlgInfo.RoundKey;
  let C = BIG_B2D(Data, 8);
  let D = BIG_B2D(Data, 12);
  let A = BIG_B2D(Data, 0);
  ...
  res = SeedRound(A, B, C, D, K, 30);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 28);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 26);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 24);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 22);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 20);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 18);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 16);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 14);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 12);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 10);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 8);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 6);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 4);
  C = res.A;
  D = res.B;
  res = SeedRound(A, B, C, D, K, 2);
  A = res.A;
  B = res.B;
  res = SeedRound(C, D, A, B, K, 0);
  C = res.A;
  D = res.B;
